...
class HiCMatrix(object):
    ...
    def __init__(self, matrix_file, name="", size="auto", n_contigs=1,
        ...
        if self.positions == "auto":
            self.positions = [int(j) for i in range(self.n_contigs)
                              for j in np.linspace(0, self.size * KB_BINNING, self.size)]
        elif isinstance(self.positions, (int, float, np.int32, np.int64, np.float32, np.float64)):
            binning = self.positions
            self.positions = [int(j) for i in range(self.n_contigs)
                              for j in np.linspace(0, self.size * binning, self.size)]
            self.level = binning
    ...
    def _init_matrix(self, matrix_file):
        ...
        if isinstance(matrix_file, basestring):
            try:
                matrix = np.genfromtxt(
                    matrix_file, dtype=None, filling_values=0)
                print("test")
        ...
        if len(matrix.shape) < 2 and len(matrix) > 1:
            ...
            matrix.shape = (1, 1)
        is_sparse = (min(matrix.shape) == 3 and max(matrix.shape)
                     != 3) or hasattr(matrix, "getformat")
        is_horizontal = is_sparse and matrix.shape[0] > matrix.shape[1]
        is_square = (not is_sparse and matrix.shape[0] == matrix.shape[1]) or (
            is_sparse and np.amax(matrix.row) == np.amax(matrix.col))
        if not is_square:
            warnings.warn(
                "Input matrix was found not to be square. I will try to sort out the extra lines, but things may be messy.", RuntimeWarning)
    ...
    def _init_from_dade_matrix(self, matrix, *args, **kwargs):
        ...
        parsed_header = list(
            zip(*[str(h)[:-1].strip('"').strip("'").split("~") for h in header[1:]]))
        if len(parsed_header) == 2:
            contigs, positions = parsed_header
        else:
            global_index, contigs, index, positions, _ = parsed_header

        # yields somethings like {'subtilis_chr1': 1, 'vibrio_plasmid1:'2'},
        # and so on
        contig_map = dict(zip(OrderedDict.fromkeys(contigs),
                              np.array(range(1, len(set(contigs)) + 1))))
        ...
        try:
            ...
            if load_full and isinstance(genome, types.GeneratorType):
                ...
                if len(self.genome) != len(set(self.contigs)):
                    warnings.warn(
                        "Warning, genome and contig information mismatch.", RuntimeWarning)
            else:
                self.genome = genome
    ...
    def _get_contig(self, index):
        ...
        new_annotations = {name: annotation[start:end]
    ...
    def _get_slice(self, index):
        if index.stop >= index.start:
            new_matrix = self.matrix[
                index.start:index.stop:index.step, index.start:index.stop:index.step]
        else:
            new_matrix = np.array([0])
        new_size = len(new_matrix)
        new_positions = self.positions[index.start:index.stop:index.step]
        new_annotations = {name: annotation[
            index.start:index.stop:index.step] for name, annotation in self.annotations.items()}
        ...
        if isinstance(self.genome, list) and self.genome[0]:
            new_genome = [self.genome[first_contig]
                          [self.positions[index.start]:]]
            new_genome += [self.genome[contig]
                           for contig in range(first_contig + 1, last_contig)]
            new_genome += [self.genome[last_contig]
                           [:self.positions[index.stop]]]
    ...
    def trim(self, thresh, n_std=3, s_min=None, s_max=None):
        ...
        new_matrix = M[f][:, f]
        new_size = len(f)
        new_annotations = {key: annotation[f]
